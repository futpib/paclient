...
function parseServerString(serverString) {
  const returnValue = [];
  const addrs = serverString.split(/[ \t]+/g);
  for (const addr of addrs) {
    var m = RE_SERVER_STRING.exec(addr);
    if (m !== null) {
      ...
      if (unixPath) {
        returnValue.push(['unix', unixPath]);
      } else {
        ...
        if (ipType === 0) {
          ...
          if (m !== null) {
            ...
            if (ipType === 0) {
              // hostname:port
              returnValue.push(['host', tcpType, ip, port]);
            } else {
              // ip:port
              returnValue.push(['ip', ip, port]);
            }
          } else {
            // hostname
            returnValue.push(['host', tcpType, tcpAddrPort, DEFAULT_PORT]);
          }
        } else {
          // ip
          returnValue.push(['ip', tcpAddrPort, DEFAULT_PORT]);
        }
      }
    }
  }
...
function parseConfig(path) {
  try {
    ...
    for (const line of lines) {
      ...
      switch (key) {
        case 'default-server':
          returnValue.addrs = parseServerString(value);
          break;
        case 'cookie-file':
          returnValue.cookieFile = value;
          break;
        case 'auto-connect-localhost':
        case 'auto-connect-display':
          returnValue[key] = (value === '1' || value === 'yes' || value === 'on'
                      || value === 'true');
          break;
      }
    }
...
function readCookie(hexString) {
  if (hexString.length !== 512)
    return null;
  const cookie = Buffer.from(hexString, 'hex');
