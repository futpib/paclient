...
PAClient.prototype.getModuleByIndex = function(index, cb) {
  ...
  buf[25] = PA_TAG_U32;
  buf.writeUInt32BE(addRequest(this, cmd, cb), 26, true);
...
PAClient.prototype.getSink = function(criteria, cb) {
  if (typeof criteria === 'number') {
    ...
    throw new TypeError('Sink criteria must be a name or index');
  }
  ...
  if (typeof criteria === 'number') {
    ...
    var length = Buffer.byteLength(criteria);
    if (length > 0)
      ++length;
    buf = Buffer.allocUnsafe(36 + length);
  }
...
PAClient.prototype.getSource = function(criteria, cb) {
  if (typeof criteria === 'number') {
    ...
    throw new TypeError('Source criteria must be a name or index');
  }
...
PAClient.prototype.getCard = function(criteria, cb) {
  if (typeof criteria === 'number') {
    ...
    throw new TypeError('Card criteria must be a name or index');
  }
...
PAClient.prototype.setSinkVolumes = function(criteria, levels, cb) {
  if (typeof criteria === 'number') {
    ...
    throw new TypeError('Volume levels argument must be an array');
...
PAClient.prototype.setSinkMute = function(criteria, muted, cb) {
  if (typeof criteria === 'number') {
    ...
    throw new TypeError('Muted argument must be a boolean');
  }
  ...
  if (typeof criteria === 'number') {
    ...
    if (length > 0)
      ++length;
    buf = Buffer.allocUnsafe(37 + length);
  }
...
PAClient.prototype.setSinkSuspend = function(criteria, suspended, cb) {
  if (typeof criteria === 'number') {
    ...
    throw new TypeError('Suspended argument must be a boolean');
  }
...
PAClient.prototype.setDefaultSinkByName = function(name, cb) {
  ...
  if (nameLength > 0)
    ++nameLength;
  const buf = Buffer.allocUnsafe(31 + nameLength);
...
PAClient.prototype.moveSinkInput = function(sinkInputIndex, destSink, cb) {
  if (typeof sinkInputIndex !== 'number' || sinkInputIndex < 0
    ...
    throw new TypeError('Destination sink must be a name or index');
  }
  ...
  if (typeof destSink === 'number') {
    ...
    var length = Buffer.byteLength(destSink);
    if (length > 0)
      ++length;
    buf = Buffer.allocUnsafe(41 + length);
  }
...
function(sourceOutputIndex, destSource, cb) {
  if (typeof sourceOutputIndex !== 'number' || sourceOutputIndex < 0
    ...
    throw new TypeError('Destination source must be a name or index');
  }
...
PAClient.prototype.setSinkPort = function(sink, portName, cb) {
  if (typeof sink === 'number') {
    ...
    throw new TypeError('Sink must be a name or index');
  } else if (typeof portName !== 'string') {
    throw new TypeError('Port name must be a string');
  }
...
PAClient.prototype.setSourcePort = function(source, portName, cb) {
  if (typeof source === 'number') {
    ...
    throw new TypeError('Source must be a name or index');
  } else if (typeof portName !== 'string') {
    throw new TypeError('Port name must be a string');
  }
...
PAClient.prototype.setCardProfile = function(card, profile, cb) {
  if (typeof card === 'number') {
    ...
    throw new TypeError('Card must be a name or index');
...
PAClient.prototype.updateClientProperties = function(props, mode, cb) {
  ...
  const propLength = calcPropArrayLength(propArray);

  const buf = Buffer.allocUnsafe(35 + propLength);
...
PAClient.prototype.removeClientProperties = function(keys, cb) {
  ...
  for (const key of keys) {
    ...
    if (key.length > 0)
      namesLength += 2 + Buffer.byteLength(key);
  }

  const buf = Buffer.allocUnsafe(31 + namesLength);
...
PAClient.prototype.loadModule = function(name, argument, cb) {
  ...
  if (typeof argument !== 'string') {
    throw new TypeError('Module argument must be a string (at least pass an empty string)');
  }
...
PAClient.prototype.unloadModuleByIndex = function(index, cb) {
  if (typeof index !== 'number') {
    throw new TypeError('Module index must be a number');
  }
  ...
  for (const event of events) {
    switch (event) {
      ...
      case 'all':
        ...
        break;
      default:
        throw new Error('Invalid event category: ' + event);
    }
  }
...
function nextAddr(client, addrs, cookie, props) {
  ...
  if (addr[0] === 'host' && addr[1] !== undefined) {
    const options = (addr[1] === 4 ? DNS_4ONLY : DNS_6ONLY);
    lookup(addr[2], options, (err, ip) => {
      if (err) {
        nextAddr(client, addrs, cookie);
        return;
      }
...
function readLength() {
  ...
  if (header === null) {
    ...
    const length = header.readUInt32BE(0, true);
...
function parsePayload(client, payload) {
  ...
  if (type === PA_COMMAND_ERROR || type === PA_COMMAND_REPLY) {
    var request;
    if (reqid === 0xFFFFFFFF || (request = client._reqs[reqid]) === undefined)
      return malformedPacket(client);
    delete client._reqs[request];
    const cb = request.cb;
    if (typeof cb !== 'function')
      return;
    if (type === PA_COMMAND_ERROR) {
      const returnValue = parseError(payload);
      if (returnValue === undefined)
        return malformedPacket(client);
      cb(returnValue);
    } else {
      const returnValue = parseReply(payload, request.cmd, client._serverVer);
      if (returnValue === undefined)
        return malformedPacket(client);
      cb(null, returnValue);
    }
...
function parseError(payload) {
  ...
  const errMessage = errors[errCode];
  if (errMessage === undefined)
    errMessage = `Unknown error code ${errCode}`;
...
function parseReply(payload, cmd, serverVer) {
  ...
  switch (cmd) {
    case PA_COMMAND_AUTH:
      if (length < 15 || payload[10] !== PA_TAG_U32)
        return;
    ...
    case PA_COMMAND_GET_MODULE_INFO_LIST: {
      ...
      p = 10;
      while (p < length) {
        var mod = readModule(payload, p, length, serverVer);
        if (mod === undefined)
          return;
        [p, mod] = mod;
        modules.push(mod);
    ...
    case PA_COMMAND_GET_CLIENT_INFO_LIST: {
      ...
      p = 10;
      while (p < length) {
        var client = readClient(payload, p, length, serverVer);
        if (client === undefined)
          return;
        [p, client] = client;
        clients.push(client);
    ...
    case PA_COMMAND_GET_SINK_INFO_LIST: {
      ...
      p = 10;
      while (p < length) {
        var sink = readSink(payload, p, length, serverVer);
        if (sink === undefined)
          return;
        [p, sink] = sink;
        sinks.push(sink);
    ...
    case PA_COMMAND_GET_SOURCE_INFO_LIST: {
      ...
      p = 10;
      while (p < length) {
        var source = readSource(payload, p, length, serverVer);
        if (source === undefined)
          return;
        [p, source] = source;
        sources.push(source);
    ...
    case PA_COMMAND_GET_SINK_INPUT_INFO_LIST: {
      ...
      p = 10;
      while (p < length) {
        var sinkInput = readSinkInput(payload, p, length, serverVer);
    ...
    case PA_COMMAND_GET_SOURCE_OUTPUT_INFO_LIST: {
      ...
      p = 10;
      while (p < length) {
        var sourceOutput = readSourceOutput(payload, p, length, serverVer);
    ...
    case PA_COMMAND_GET_CARD_INFO_LIST: {
      ...
      p = 10;
      while (p < length) {
        var card = readCard(payload, p, length, serverVer);
        if (card === undefined)
          return;
        [p, card] = card;
        cards.push(card);
    ...
    case PA_COMMAND_GET_SERVER_INFO: {
      p = 10;
      var packageName = readString(payload, p, length);
      if (packageName === undefined)
        return;
      [p, packageName] = packageName;
      var packageVersion = readString(payload, p, length);
      if (packageVersion === undefined)
        return;
      [p, packageVersion] = packageVersion;
      var username = readString(payload, p, length);
      if (username === undefined)
        return;
      [p, username] = username;
      var hostname = readString(payload, p, length);
      if (hostname === undefined)
        return;
      [p, hostname] = hostname;
      var defaultSampleSpec = readSampleSpec(payload, p, length);
      if (defaultSampleSpec === undefined)
        return;
      [p, defaultSampleSpec] = defaultSampleSpec;
      var defaultSinkName = readString(payload, p, length);
      if (defaultSinkName === undefined)
        return;
      [p, defaultSinkName] = defaultSinkName;
      var defaultSourceName = readString(payload, p, length);
      if (defaultSourceName === undefined)
        return;
      [p, defaultSourceName] = defaultSourceName;
      if (p + 5 > length || payload[p++] !== PA_TAG_U32)
        return;
      const cookie = payload.readUInt32BE(p, true);
      p += 4;
      if (serverVer >= 15) {
        var defaultChannelMap = readChannelMap(payload, p, length);
    ...
    case PA_COMMAND_GET_MODULE_INFO: {
      const mod = readModule(payload, 10, length, serverVer);
      if (mod === undefined)
        return;
      return mod[1];
    }
    ...
    case PA_COMMAND_GET_CLIENT_INFO: {
      const client = readClient(payload, 10, length, serverVer);
      if (client === undefined)
        return;
      return client[1];
    }
    ...
    case PA_COMMAND_GET_SINK_INFO: {
      const sink = readSink(payload, 10, length, serverVer);
      if (sink === undefined)
        return;
      return sink[1];
    }
    ...
    case PA_COMMAND_GET_SOURCE_INFO: {
      const source = readSource(payload, 10, length, serverVer);
      if (source === undefined)
        return;
      return source[1];
    }
    ...
    case PA_COMMAND_GET_SINK_INPUT_INFO: {
      const sinkInput = readSinkInput(payload, 10, length, serverVer);
      if (sinkInput === undefined)
        return;
      return sinkInput[1];
    }
    ...
    case PA_COMMAND_GET_SOURCE_OUTPUT_INFO: {
      const sourceOutput = readSourceOutput(payload, 10, length, serverVer);
      if (sourceOutput === undefined)
        return;
    ...
    case PA_COMMAND_GET_CARD_INFO: {
      const card = readCard(payload, 10, length, serverVer);
      if (card === undefined)
        return;
      return card[1];
    }
...
function returnResult(p, value) {
  _result[0] = p;
  _result[1] = value;
  return _result;
}

function readString(payload, p, length) {
  if (p >= length)
    return;
...
function readSampleSpec(payload, p, length) {
  if (p + 7 >= length || payload[p++] !== PA_TAG_SAMPLE_SPEC)
    return;
...
function readChannelMap(payload, p, length) {
  if (p + 2 >= length || payload[p++] !== PA_TAG_CHANNEL_MAP)
    return;
  const channels = payload[p++];
...
function readChannelVolumes(payload, p, length) {
  if (p + 2 >= length || payload[p++] !== PA_TAG_CVOLUME)
    return;
  const channels = payload[p++];
  if (p + (channels * 4) > length)
    return;
  const vols = [];
...
function readProplist(payload, p, length) {
  if (p >= length || payload[p++] !== PA_TAG_PROPLIST)
    return;
  const props = Object.create(null);
  while (true) {
    var key = readString(payload, p, length);
    if (key === undefined)
      return;
    [p, key] = key;
    if (key.length === 0)
      break;
    if (p + 10 > length || payload[p] !== PA_TAG_U32
        || payload[p + 5] !== PA_TAG_ARBITRARY) {
      return;
    }

    const valueLength = payload.readUInt32BE(p + 1, true);
    const arbLength = payload.readUInt32BE(p + 6, true);
    if (valueLength !== arbLength)
      return;
    p += 10;
    if (p + valueLength >= length)
      return;
...
function readModule(payload, p, length, serverVer) {
  if (p + 5 >= length || payload[p++] !== PA_TAG_U32)
    return;
  const index = payload.readUInt32BE(p, true);
  p += 4;
  var name = readString(payload, p, length);
  if (name === undefined)
    return;
  [p, name] = name;
  var args = readString(payload, p, length);
  if (args === undefined)
    return;
  [p, args] = args;
  if (p + 5 >= length || payload[p] !== PA_TAG_U32)
  ...
  p += 5;
  if (serverVer >= 15) {
    var properties = readProplist(payload, p, length);
...
function readClient(payload, p, length, serverVer) {
  if (p + 5 >= length || payload[p++] !== PA_TAG_U32)
    return;
  const index = payload.readUInt32BE(p, true);
  p += 4;
  ...
  if (moduleIndex === 0xFFFFFFFF)
    moduleIndex = -1;
  p += 5;
  var driverName = readString(payload, p, length);
...
function readSink(payload, p, length, serverVer) {
  if (p + 5 >= length || payload[p++] !== PA_TAG_U32)
    return;
  const index = payload.readUInt32BE(p, true);
  p += 4;
  ...
  if (name === undefined)
    return;
  [p, name] = name;
  var description = readString(payload, p, length);
  if (description === undefined)
    return;
  [p, description] = description;
  var sampleSpec = readSampleSpec(payload, p, length);
  if (sampleSpec === undefined)
    return;
  [p, sampleSpec] = sampleSpec;
  var channelMap = readChannelMap(payload, p, length);
  ...
  if (moduleIndex === 0xFFFFFFFF)
    moduleIndex = -1;
  p += 5;
  var channelVolumes = readChannelVolumes(payload, p, length);
  if (channelVolumes === undefined)
    return;
  [p, channelVolumes] = channelVolumes;
  if (p + 6 >= length || payload[p + 1] !== PA_TAG_U32)
    return;
  var muted;
  switch (payload[p]) {
  ...
  p += 4;
  var monitorSourceName = readString(payload, p, length);
  if (monitorSourceName === undefined)
    return;
  [p, monitorSourceName] = monitorSourceName;
  if (p + 9 >= length || payload[p++] !== PA_TAG_USEC)
    return;
  ...
  if (serverVer >= 15) {
    if (p + 20 > length || payload[p] !== PA_TAG_VOLUME
  ...
  if (serverVer >= 16) {
    if (p + 5 >= length || payload[p++] !== PA_TAG_U32)
      return;
    const numberPorts = payload.readUInt32BE(p, true);
    var ports = [];
    p += 4;
    for (var i = 0; i < numberPorts; ++i) {
      var portName = readString(payload, p, length);
      if (portName === undefined)
        return;
      [p, portName] = portName;
      var portDescription = readString(payload, p, length);
    ...
    var activePortName = readString(payload, p, length);
  ...
  if (serverVer >= 21) {
    if (p + 2 > length || payload[p++] !== PA_TAG_U8)
      return;
    const numberFormats = payload[p++];
    var formats = [];
    for (var i = 0; i < numberFormats; ++i) {
      if (p + 3 >= length || payload[p++] !== PA_TAG_FORMAT_INFO
          || payload[p++] !== PA_TAG_U8) {
        return;
      }

      const encoding = payload[p++];
      var formatProperties = readProplist(payload, p, length);
...
function readSource(payload, p, length, serverVer) {
  if (p + 5 >= length || payload[p++] !== PA_TAG_U32)
    return;
  const index = payload.readUInt32BE(p, true);
  p += 4;
...
function readSinkInput(payload, p, length, serverVer) {
  if (p + 5 >= length || payload[p++] !== PA_TAG_U32)
    return;
  ...
  var sinkLatency = (payload[p++] * 72057594037927940)
                     ...
                     + (payload[p++] * 256)
                     + payload[p++];
  var resampleMethod = readString(payload, p, length);
...
function readSourceOutput(payload, p, length, serverVer) {
  if (p + 5 >= length || payload[p++] !== PA_TAG_U32)
    return;
  var index = payload.readUInt32BE(p, true);
  if (index === 0xFFFFFFFF)
...
function readCard(payload, p, length, serverVer) {
  if (p + 5 >= length || payload[p++] !== PA_TAG_U32)
    return;
  var index = payload.readUInt32BE(p, true);
  if (index === 0xFFFFFFFF)
  ...
  if (p + 5 >= length || payload[p++] !== PA_TAG_U32)
    return;
  var numberProfiles = payload.readUInt32BE(p, true);
  var profiles = [];
  p += 4;
  for (var i = 0; i < numberProfiles; ++i) {
    var profileName = readString(payload, p, length);
    if (profileName === undefined)
      return;
    [p, profileName] = profileName;
    var profileDescription = readString(payload, p, length);
    if (profileDescription === undefined)
      return;
    [p, profileDescription] = profileDescription;
    if (p + 15 > length || payload[p] !== PA_TAG_U32
        || payload[p + 5] !== PA_TAG_U32
        || payload[p + 10] !== PA_TAG_U32) {
      return;
    }

    const numberSinks = payload.readUInt32BE(++p, true);
    const numberSources = payload.readUInt32BE(p += 5, true);
    ...
    profiles.push({
      ...
      numSinks: numberSinks,
      numSources: numberSources,
      priority,
      available: profileAvailable
    });
  }

  var activeProfileName = readString(payload, p, length);
  ...
  if (serverVer >= 26) {
    if (p + 5 > length || payload[p++] !== PA_TAG_U32)
      return;
    var numberPorts = payload.readUInt32BE(p, true);
    var ports = [];
    p += 4;
    for (var i = 0; i < numberPorts; ++i) {
      var portName = readString(payload, p, length);
      ...
      if (portDescription === undefined)
        return;
      [p, portDescription] = portDescription;
      if (p + 12 > length || payload[p] !== PA_TAG_U32
          || payload[p + 5] !== PA_TAG_U32
      ...
      var portProperties = readProplist(payload, ++p, length);
      if (portProperties === undefined)
        return;
      [p, portProperties] = portProperties;
      if (p + 5 > length || payload[p++] !== PA_TAG_U32)
        return;
      const numberProfiles = payload.readUInt32BE(p, true);
      const portProfiles = [];
      p += 4;
      for (var j = 0; j < numberProfiles; ++j) {
        var portProfileName = readString(payload, p, length);
      ...
      if (serverVer >= 27) {
        if (p + 9 > length || payload[p++] !== PA_TAG_S64)
          return;
...
function addRequest(client, cmd, cb) {
  const number = client._reqid++;
  if (number === 0xFFFFFFFF)
    client._reqid = 0;
  client._reqs[number] = { cmd, cb };
  return number;
}

function propListToArray(list, returnValue, prefix) {
  ...
  for (const key of keys) {
    if (key.length > 0) {
      ...
      if (typeof value === 'object' && value !== null)
        propListToArray(value, returnValue, (prefix ? `${prefix}.${key}` : key));
      else if (typeof value === 'string' || Buffer.isBuffer(value))
        returnValue.push((prefix ? `${prefix}.${key}` : key), value);
      else
        throw new Error('Invalid property list value type: ' + typeof value);
    }
  }
...
function calcPropArrayLength(props) {
  var length = 2;
  for (var i = 0; i < props.length; i += 2) {
    const key = props[i];
    const value = props[i + 1];
    length += 12 + Buffer.byteLength(key);
...
function writePropArray(props, payload, p) {
  ...
  for (var i = 0; i < props.length; i += 2) {
    ...
    if (typeof value === 'string') {
      ...
      payload.writeUInt32BE(length, p, true);
      p += 4;
      payload[p++] = PA_TAG_ARBITRARY;
      payload.writeUInt32BE(length, p, true);
      p += 4;
      p += payload.write(value, p, length - 1);
      payload[p++] = 0; // NULL terminator
    } else {
...
function sendSimplePacket(client, cmd, cb) {
  ...
  buf.writeUInt32BE(cmd, 21, true);
  buf[25] = PA_TAG_U32;
  buf.writeUInt32BE(addRequest(client, cmd, cb), 26, true);
...
function sendAuthPacket(client, cookie, cb) {
  ...
  const buf = Buffer.allocUnsafe(30 + (2 * 4) + 2 + cookieLength);
  ...
  buf.writeUInt32BE(cookieLength, 36, true);
  if (cookieLength > 0)
    cookie.copy(buf, 40);
...
function sendClientNamePacket(client, props, cb) {
  ...
  if (client._serverVer >= 13) {
    const propArray = propListToArray(props);
    const propLength = calcPropArrayLength(propArray);
    buf = Buffer.allocUnsafe(30 + propLength);
    ...
    const nameLength = Buffer.byteLength(props.application.name);
    buf = Buffer.allocUnsafe(32 + nameLength);
    buf[30] = PA_TAG_STRING;
